...
import org.apache.kafka.storage.diskless.{DisklessBrokerSelector, DisklessClientZone, DisklessLogMetadata}
...
import java.util.concurrent.{Callable, CompletableFuture, ConcurrentHashMap, CountDownLatch, Future, RejectedExecutionException, TimeUnit}
...
class ReplicaManagerTest {
  ...
  private def findDisklessTopicIdForOwner(targetBrokerId: Int, aliveBrokerIds: Seq[Int], partition: Int = 0): Uuid = {
    val selector = new DisklessBrokerSelector(
      _ => aliveBrokerIds.sorted.map(id => new Node(id, s"host$id", id)).asJava,
    ...
    Iterator.continually(Uuid.randomUuid()).take(maxAttempts).find { topicId =>
      ...
      fail(
        s"Unable to find a diskless topicId for broker $targetBrokerId " +
          s"after $maxAttempts attempts. aliveBrokerIds=$aliveBrokerIds, partition=$partition"
      )
    }
  }
  ...
  private def setupReplicaManagerWithMockedPurgatories(
    ...
    mockReplicaManagerInterceptor: Option[ReplicaManagerInterceptor] = None,
    ...
    new ReplicaManager(
      ...
      override protected def createInterceptor(): ReplicaManagerInterceptor =
        mockReplicaManagerInterceptor.getOrElse(super.createInterceptor())
  ...
  @Test
  def testDeltaRemovedTopicDoesNotDirectlyCleanupDisklessStorageSupport(): Unit = {
    val localId = 1
    ...
    when(mockDisklessStorageSupport.trackedTopicNames()).thenReturn(Collections.emptySet())
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      timer = new MockTimer(time),
      brokerId = localId,
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport)
  ...
  @Test
  def testDeltaRemovedReplicaDoesNotDirectlyCleanupDisklessStorageSupport(): Unit = {
    val localId = 1
    val topicPartition = new TopicPartition("diskless-topic", 0)
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    when(mockDisklessStorageSupport.isDisklessStorageTopic(topicPartition.topic)).thenReturn(true)
    ...
    try {
      ...
      reassignmentDelta.replay(new PartitionChangeRecord()
        .setPartitionId(0)
        .setTopicId(FOO_UUID)
        .setReplicas(util.Arrays.asList(localId + 1, localId + 2))
        .setIsr(util.Arrays.asList(localId + 1, localId + 2))
        .setLeader(localId + 1)
      )
      val reassignmentImage = imageFromTopics(reassignmentDelta.apply())
      replicaManager.applyDelta(reassignmentDelta, reassignmentImage)
  ...
  @Test
  def testDisklessFollowerTransitionDoesNotDirectlyCleanupStorageSupport(): Unit = {
    val localId = 1
    val topicPartition = new TopicPartition("diskless-topic", 0)
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    when(mockDisklessStorageSupport.isDisklessStorageTopic(topicPartition.topic)).thenReturn(true)
  ...
  @Test
  def testClassicLeadershipAcquisitionStartsTailPublicationWithoutAppend(): Unit = {
    val localId = 0
    val classicTopic = "classic-acquired-tail-topic"
    val classicTopicId = Uuid.randomUuid()
    val topicPartition = new TopicPartition(classicTopic, 0)
    val topicIdPartition = new TopicIdPartition(classicTopicId, topicPartition)
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])

    when(mockDisklessStorageSupport.isDisklessStorageTopic(classicTopic)).thenReturn(false)
    when(mockDisklessStorageSupport.currentClassicPublisherGeneration(topicIdPartition)).thenReturn(17L)

    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      timer = new MockTimer(time),
      brokerId = localId,
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport),
      mockReplicaManagerInterceptor = Some(mockInterceptor)
    )
    ...
    try {
      val leaderDelta = topicsCreateDelta(
        ...
        partitions = List(0),
        topicName = classicTopic,
        topicId = classicTopicId)

      replicaManager.applyDelta(leaderDelta, imageFromTopics(leaderDelta.apply()))

      verify(mockDisklessStorageSupport).currentClassicPublisherGeneration(topicIdPartition)
      verify(mockInterceptor).onLeadershipAcquired(
        replicaManager.getPartitionOrException(topicPartition), 17L)
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  }
  ...
  @Test
  def testClassicLeadershipLossFencesTailPublicationImmediately(): Unit = {
    val localId = 0
    val classicTopic = "classic-lost-tail-topic"
    val classicTopicId = Uuid.randomUuid()
    val topicPartition = new TopicPartition(classicTopic, 0)
    val topicIdPartition = new TopicIdPartition(classicTopicId, topicPartition)
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    ...
    try {
      ...
      val leaderImage = imageFromTopics(leaderDelta.apply())
      replicaManager.applyDelta(leaderDelta, leaderImage)
      clearInvocations(mockInterceptor)

      val followerDelta = new TopicsDelta(leaderImage.topics())
      followerDelta.replay(new PartitionChangeRecord()
        .setPartitionId(0)
        .setTopicId(classicTopicId)
        .setReplicas(util.Arrays.asList(localId, localId + 1))
        .setIsr(util.Arrays.asList(localId, localId + 1))
        .setLeader(localId + 1))
      replicaManager.applyDelta(followerDelta, imageFromTopics(followerDelta.apply()))

      verify(mockInterceptor).onLeadershipLost(topicIdPartition)
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  }
  ...
  @Test
  def testDisklessAppendNotifiesInterceptorOnlyAfterSuccessfulDurableAppend(): Unit = {
    val disklessTopic = "diskless-compaction-topic"
    val successPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition(disklessTopic, 0))
    val failedPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition(disklessTopic, 1))
    val records = TestUtils.records(Seq(new SimpleRecord("value".getBytes)))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    ...
    when(mockDisklessStorageSupport.partitionAppendEntries(any())).thenAnswer(invocation => {
      val entries = invocation.getArgument[util.Map[TopicIdPartition, MemoryRecords]](0)
      new DisklessStorageReplicaManagerSupport.PartitionedEntries[TopicIdPartition, MemoryRecords](
        new util.LinkedHashMap[TopicIdPartition, MemoryRecords](entries),
        new util.LinkedHashMap[TopicIdPartition, MemoryRecords]())
    })
    when(mockDisklessStorageSupport.handleAppend(any(), ArgumentMatchers.isNull())).thenReturn(
      CompletableFuture.completedFuture(Map(
        successPartition -> new PartitionResponse(Errors.NONE),
        failedPartition -> new PartitionResponse(Errors.KAFKA_STORAGE_ERROR)).asJava)
    )
    when(mockDisklessStorageSupport.logMetadata(successPartition, 7L)).thenReturn(
      CompletableFuture.completedFuture(Optional.of(new DisklessLogMetadata(73L, 19L))))
    when(mockDisklessStorageSupport.currentDisklessOwnershipGeneration(successPartition)).thenReturn(7L)
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(successPartition, 7L)).thenReturn(true)
    ...
    try {
      val result = new CallbackResult[util.Map[TopicIdPartition, PartitionResponse]]()
      replicaManager.appendRecords(
        timeout = 1000,
        requiredAcks = 1,
        internalTopicsAllowed = false,
        origin = AppendOrigin.CLIENT,
        entriesPerPartition = Map(successPartition -> records, failedPartition -> records),
        responseCallback = result.fire)

      assertEquals(Errors.NONE, result.assertFired.get(successPartition).error)
      assertEquals(Errors.KAFKA_STORAGE_ERROR, result.assertFired.get(failedPartition).error)
      verify(mockDisklessStorageSupport).logMetadata(successPartition, 7L)
      verify(mockDisklessStorageSupport, never()).logMetadata(
        ArgumentMatchers.eq(failedPartition), anyLong())
      verify(mockInterceptor).onDisklessAppend(successPartition, 73L, 19L, 7L)
      verify(mockInterceptor, never()).onDisklessAppend(
        ArgumentMatchers.eq(failedPartition), anyLong(), anyLong(), anyLong())
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  }
  ...
  @Test
  def testDisklessOwnershipAcquisitionWakesPendingTailWithoutAppend(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-acquired-tail-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 7L))
      .thenReturn(true)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 7L)).thenReturn(
      CompletableFuture.completedFuture(Optional.of(new DisklessLogMetadata(73L, 19L))))
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      timer = new MockTimer(time),
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport),
    ...
    try {
      replicaManager.notifyDisklessOwnershipAcquired(topicIdPartition, 7L)

      verify(mockDisklessStorageSupport).logMetadata(topicIdPartition, 7L)
      verify(mockInterceptor).onDisklessAppend(topicIdPartition, 73L, 19L, 7L)
      assertEquals(0, replicaManager.pendingDisklessAppendNotificationCount)
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  }
  ...
  @Test
  def testDisklessOwnershipAcquisitionMetadataFailureRetriesWithoutAppend(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-acquired-tail-retry-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val retryTime = new MockTime()
    val retryScheduler = new MockScheduler(retryTime)
    val failedMetadata = new CompletableFuture[Optional[DisklessLogMetadata]]()
    failedMetadata.completeExceptionally(new RuntimeException("transient metadata failure"))
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 7L))
      .thenReturn(true)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 7L)).thenReturn(
      failedMetadata,
    ...
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      ...
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport),
      mockReplicaManagerInterceptor = Some(mockInterceptor),
      schedulerOverride = Some(retryScheduler)
    )

    try {
      replicaManager.notifyDisklessOwnershipAcquired(topicIdPartition, 7L)
      verify(mockInterceptor, never()).onDisklessAppend(
        ArgumentMatchers.eq(topicIdPartition), anyLong(), anyLong(), anyLong())

      retryTime.sleep(1000L)
      retryScheduler.tick()

      verify(mockDisklessStorageSupport, times(2)).logMetadata(topicIdPartition, 7L)
      verify(mockInterceptor).onDisklessAppend(topicIdPartition, 73L, 19L, 7L)
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  ...
  @Test
  def testDisklessOwnershipLossImmediatelyRemovesHungMetadataNotification(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-hung-tail-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val metadataFuture = new CompletableFuture[Optional[DisklessLogMetadata]]()
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 7L))
      .thenReturn(true, false)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 7L)).thenReturn(metadataFuture)
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      timer = new MockTimer(time),
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport),
      mockReplicaManagerInterceptor = Some(mockInterceptor)
    ...
    try {
      replicaManager.notifyDisklessOwnershipAcquired(topicIdPartition, 7L)
      assertEquals(1, replicaManager.pendingDisklessAppendNotificationCount)

      replicaManager.onDisklessPartitionOwnershipLost(topicIdPartition, 8L)

      assertEquals(0, replicaManager.pendingDisklessAppendNotificationCount)
      verify(mockInterceptor).onPartitionOwnershipLost(topicIdPartition, 8L)

      metadataFuture.complete(Optional.of(new DisklessLogMetadata(73L, 19L)))
      verify(mockInterceptor, never()).onDisklessAppend(
        ArgumentMatchers.eq(topicIdPartition), anyLong(), anyLong(), anyLong())
    } finally {
      replicaManager.shutdown(checkpointHW = false)
  ...
  @Test
  def testStaleOwnershipLossDoesNotRemoveReacquiredMetadataNotification(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-reacquired-tail-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val metadataFuture = new CompletableFuture[Optional[DisklessLogMetadata]]()
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 9L))
      .thenReturn(true)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 9L)).thenReturn(metadataFuture)
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      timer = new MockTimer(time),
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport),
      mockReplicaManagerInterceptor = Some(mockInterceptor)
    ...
    try {
      replicaManager.notifyDisklessOwnershipAcquired(topicIdPartition, 9L)
      replicaManager.onDisklessPartitionOwnershipLost(topicIdPartition, 8L)

      assertEquals(1, replicaManager.pendingDisklessAppendNotificationCount)
      metadataFuture.complete(Optional.of(new DisklessLogMetadata(73L, 19L)))
      verify(mockInterceptor).onDisklessAppend(topicIdPartition, 73L, 19L, 9L)
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  }
  ...
  @Test
  def testDisklessAppendMetadataFailureRetriesWithoutAnotherAppend(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-retry-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val retryTime = new MockTime()
    val retryScheduler = new MockScheduler(retryTime)
    ...
    when(mockDisklessStorageSupport.currentDisklessOwnershipGeneration(topicIdPartition)).thenReturn(7L)
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 7L)).thenReturn(true)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 7L)).thenReturn(
      failedMetadata,
      CompletableFuture.completedFuture(Optional.of(new DisklessLogMetadata(73L, 19L))))
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
    ...
    try {
      replicaManager.notifySuccessfulDisklessAppends(
        Map(topicIdPartition -> new PartitionResponse(Errors.NONE)))
  ...
  @Test
  def testDisklessMetadataRetrySchedulingFailureDiscardsPendingNotification(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-retry-rejection-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val rejectingScheduler = mock(classOf[Scheduler])
    val failedMetadata = new CompletableFuture[Optional[DisklessLogMetadata]]()
    failedMetadata.completeExceptionally(new RuntimeException("transient metadata failure"))

    when(mockDisklessStorageSupport.currentDisklessOwnershipGeneration(topicIdPartition)).thenReturn(7L)
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 7L)).thenReturn(true)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 7L)).thenReturn(failedMetadata)
    doThrow(new RejectedExecutionException("scheduler stopped"))
      .when(rejectingScheduler).scheduleOnce(anyString(), any[Runnable](), anyLong())
    val replicaManager = setupReplicaManagerWithMockedPurgatories(
      timer = new MockTimer(time),
      mockDisklessStorageSupport = Some(mockDisklessStorageSupport),
      mockReplicaManagerInterceptor = Some(mockInterceptor),
      schedulerOverride = Some(rejectingScheduler)
    )
  ...
  @Test
  def testShutdownClearsPendingDisklessMetadataRetry(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-retry-shutdown-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val retryScheduler = new MockScheduler(new MockTime())
    val failedMetadata = new CompletableFuture[Optional[DisklessLogMetadata]]()
    failedMetadata.completeExceptionally(new RuntimeException("transient metadata failure"))
  ...
  @Test
  def testDisklessAppendMetadataNotificationsCoalesceWhileQueryIsInFlight(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("diskless-coalesce-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val firstMetadata = new CompletableFuture[Optional[DisklessLogMetadata]]()

    when(mockDisklessStorageSupport.currentDisklessOwnershipGeneration(topicIdPartition)).thenReturn(7L)
    when(mockDisklessStorageSupport.isCurrentDisklessOwnershipGeneration(topicIdPartition, 7L)).thenReturn(true)
    when(mockDisklessStorageSupport.logMetadata(topicIdPartition, 7L)).thenReturn(
      firstMetadata,
      CompletableFuture.completedFuture(Optional.of(new DisklessLogMetadata(73L, 20L))))
    ...
    try {
      val successfulAppend = Map(topicIdPartition -> new PartitionResponse(Errors.NONE))
      replicaManager.notifySuccessfulDisklessAppends(successfulAppend)
      replicaManager.notifySuccessfulDisklessAppends(successfulAppend)
      verify(mockDisklessStorageSupport).logMetadata(topicIdPartition, 7L)

      firstMetadata.complete(Optional.of(new DisklessLogMetadata(73L, 10L)))

      verify(mockDisklessStorageSupport, times(2)).logMetadata(topicIdPartition, 7L)
      verify(mockInterceptor, never()).onDisklessAppend(topicIdPartition, 73L, 10L, 7L)
      verify(mockInterceptor).onDisklessAppend(topicIdPartition, 73L, 20L, 7L)
    } finally {
      replicaManager.shutdown(checkpointHW = false)
    }
  }
  ...
  @Test
  def testLateDisklessMetadataFromOldTopicIncarnationIsDiscarded(): Unit = {
    val topicIdPartition = new TopicIdPartition(
      Uuid.randomUuid(), new TopicPartition("recreated-diskless-topic", 0))
    val mockDisklessStorageSupport = mock(classOf[DisklessStorageReplicaManagerSupport])
    val mockInterceptor = mock(classOf[ReplicaManagerInterceptor])
    val metadataFuture = new CompletableFuture[Optional[DisklessLogMetadata]]()
  ...
  @Test
  def testDisklessAppendRedirectsWhenBrokerIsNotCurrentOwner(): Unit = {
    val localId = 0
    val targetOwner = 1
    val aliveBrokerIds = Seq(localId, targetOwner, 2)
    val disklessTopic = "diskless-redirect-topic"
    val topicPartition = new TopicPartition(disklessTopic, 0)
    val disklessTopicId = findDisklessTopicIdForOwner(targetOwner, aliveBrokerIds)
    val topicIdPartition = new TopicIdPartition(disklessTopicId, topicPartition)
    ...
    try {
      val records = TestUtils.records(Seq(new SimpleRecord("a".getBytes)))
      val result = new CallbackResult[PartitionResponse]()
      replicaManager.appendRecords(
        timeout = 1000,
        requiredAcks = -1,
  ...
  @Test
  def testDisklessFetchRedirectsWhenBrokerIsNotCurrentOwner(): Unit = {
    val localId = 0
    val targetOwner = 1
    val aliveBrokerIds = Seq(localId, targetOwner, 2)
    val disklessTopic = "diskless-fetch-redirect-topic"
    val topicPartition = new TopicPartition(disklessTopic, 0)
    val disklessTopicId = findDisklessTopicIdForOwner(targetOwner, aliveBrokerIds)
    val topicIdPartition = new TopicIdPartition(disklessTopicId, topicPartition)
    ...
    when(mockDisklessStorageSupport.partitionFetchInfos(any())).thenAnswer(invocation => {
      val entries = invocation.getArgument[util.Map[TopicIdPartition, FetchRequest.PartitionData]](0)
      new DisklessStorageReplicaManagerSupport.PartitionedEntries[TopicIdPartition, FetchRequest.PartitionData](
        new util.LinkedHashMap[TopicIdPartition, FetchRequest.PartitionData](entries),
    ...
    try {
      ...
      fetchPartitions(
        replicaManager,
        replicaId = 1,
        fetchInfos = Seq(topicIdPartition -> new PartitionData(disklessTopicId, 0, 0, 1024, Optional.empty())),
        responseCallback = results => result.fire(results.toMap.apply(topicIdPartition))
      )
  ...
  @Test
  def testDisklessListOffsetsRedirectsWhenBrokerIsNotCurrentOwner(): Unit = {
    val localId = 0
    val targetOwner = 1
    val aliveBrokerIds = Seq(localId, targetOwner, 2)
    val disklessTopic = "diskless-list-offsets-redirect-topic"
    val disklessTopicId = findDisklessTopicIdForOwner(targetOwner, aliveBrokerIds)
    ...
    try {
      ...
      when(mockDisklessStorageSupport.handleListOffsets(any(), ArgumentMatchers.eq("test-client"))).thenAnswer { invocation =>
        ...
        requests.asScala.foreach { case (tip, _) =>
          responses.put(tip, org.apache.kafka.storage.diskless.ListOffsetsPartitionResponse.error(tip, Errors.NOT_LEADER_OR_FOLLOWER))
        }
